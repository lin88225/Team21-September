...
void createDropdownArray()
{
  ...
  /* 
  two dropdowns for origin state and destination state selection 
  inci 
  */
      dropdownMap[0] = new Dropdown(SCREENX/2-DROPDOWN_WIDTH-50, SCREENY/2+4*DROPDOWN_HEIGHT, DROPDOWN_WIDTH, DROPDOWN_HEIGHT,
        "Origin State", states, HOOKERS_GREEN, COLUMBIA_BLUE, DENIM, arial, false);
      dropdownMap[1] = new Dropdown(SCREENX/2+DROPDOWN_WIDTH -200, SCREENY/2+4*DROPDOWN_HEIGHT, DROPDOWN_WIDTH, DROPDOWN_HEIGHT,
        "Destination State", states, HOOKERS_GREEN, COLUMBIA_BLUE, DENIM, arial, false);
}
/* Two functions:
...
void createScreens(int i) // i is the variable that decides which screen will be drawn
{
  ...
  } else if (i == 5)
  {
    ...
    {
     screenArray[i] = new Screen(i, dropdownMap);
